...
A linker script is the architect's blueprint for a microcontroller's memory. Without it, the linker would scatter code and data arbitrarily across the address space, and the processor would have no idea where anything lives. The script defines physical memory regions (FLASH, SRAM) with their base addresses and sizes, then assigns output sections to those regions. Every section has a VMA (Virtual Memory Address — where it lives at runtime) and an LMA (Load Memory Address — where it is stored in non-volatile memory). For read-only sections like .text and .rodata, VMA and LMA are the same (they execute directly from flash). For the .data section, VMA is in SRAM but LMA is in flash, because initial values must persist across power cycles.

This concept dates back to the earliest embedded linkers. In the 1980s, developers manually assigned addresses in assembly with ORG directives. The GNU linker (ld) introduced a powerful scripting language that became the de facto standard for ARM embedded toolchains. CMSIS standardized the section naming: .isr_vector for the vector table, .text for code, .rodata for constants, .data for initialized globals, .bss for zero-initialized globals, .heap for malloc, and .stack for the call stack. The precise placement of these sections determines whether the firmware fits in flash, whether the stack overflows into the heap, and whether DMA buffers are placed in cacheable or non-cacheable memory.

A professional firmware engineer regularly tweaks the linker script. For example, an audio application might place a large lookup table in external QSPI flash by adding a new memory region and assigning a .qspi_data section to it. A safety-critical system might put .text in two regions: critical ISR code in the tightly-coupled SRAM for deterministic timing, and non-critical code in flash. A bootloader sets the FLASH origin to 0x08000000 with 32 KB, while the application starts at 0x08008000 with the remaining flash. Getting the LMA wrong for .data — that is, specifying the wrong source address in flash — causes the startup code to copy garbage into initialized variables, one of the most subtle and frustrating bugs in embedded C.
